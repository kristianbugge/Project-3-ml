...
class FFNN:
    ...
    def fit(
        ...
        print(f"\n{scheduler.__class__.__name__}: Eta={scheduler.eta}, Lambda={lam}")
    ...
    def _backpropagate(self, X, t, lmb):
        """
        Description:
        ------------
        ...
        for i in range(len(self.weights) - 1, -1, -1):
            ...
            # regularization term
            gradient_weights += self.weights[i][1:, :] * lmb
    ...
    def cross_validation(self,
        ...
        if self.classification:
            if int(self.dimensions[-1]) != 1:
                confusion_matrix = np.zeros((int(self.dimensions[-1]), int(self.dimensions[-1])))
            else:
        ...
        for X_train, t_train, X_val, t_val in cv:
            ...
            if self.classification:
                ...
                if int(self.dimensions[-1]) != 1:
                    confusion = self.calc_multi_confusion(t_val, self.predict(X_val), int(self.dimensions[-1]))
                else:
        ...
        for i, eta in enumerate(etas):
            for j, lam in enumerate(lams):
                if scheduler == "Constant":
                    sched = Constant(eta)
                elif scheduler == "Momentum":
                    sched = Momentum(eta, momentum)
                elif scheduler == "Adagrad":
                    sched = Adagrad(eta)
                elif scheduler == "AdagradMomentum":
    ...
    def calc_multi_confusion(self, t, pred, n_classes = 10):
        """
        Description:
        ------------
            Calculates the confusion matrix given various true and predicted vlaues
        ...
        Returns:
        ------------
            I   confusion_matrix (np.ndarray) : The nxn confusion matrix
        ...
        for true_class in range(num_classes):   
            for pred_class in range(num_classes):
                confusion_matrix[true_class, pred_class] = np.sum((t == true_class) & (pred == pred_class))
